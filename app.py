...
def index():
    ...
    return render_template("index.html", items=itens, categories=categorias, filtro_nome=filtro_nome, filtro_categoria=filtro_categoria)
...
def admin_salvar(item_id=None):
    ...
    # Valida preço como número
    try:
        preco_valor = float(preco.replace(",", "."))
    except ValueError:
        flash("Preço inválido! Digite apenas números.", "error")
        return redirect(request.referrer)

    # Salva imagem enviada, se houver
...
def admin_adicionar_categoria():
    ...
    if not novo_nome:
        flash("Nome de categoria não pode ficar vazio.", "error")
        return redirect(url_for("admin"))
